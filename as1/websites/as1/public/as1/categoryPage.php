<?php
// Include the common header file to ensure consistent HTML structure and styles
require 'Header.php';
// Include the database connection file
require 'dbConn.php';

// Check if a specific category is selected via GET parameter
if (isset($_GET['category_id'])) {
    $selectedCategory = $_GET['category_id'];

    // Retrieve auctions filtered by the selected category
    $auctionsQuery = $pdo->prepare("SELECT * FROM `auctions` WHERE category_id = (SELECT category_id FROM `categories` WHERE category_id = :selectedCategory)");
    $auctionsQuery->bindParam(':selectedCategory', $selectedCategory);
    $auctionsQuery->execute();
    $auctions = $auctionsQuery->fetchAll(PDO::FETCH_ASSOC);
} else {

    // If no specific category is selected, retrieve all auctions
    $auctionsQuery = $pdo->query("SELECT * FROM `auctions`");
    $auctions = $auctionsQuery->fetchAll(PDO::FETCH_ASSOC);
}
?>
    ...
    <?php foreach ($auctions as $auction) : ?>
        <li>
            ...
            <article>
                ...
                <?php
                $conn = db_connect();
                $query = "SELECT MAX(bid) AS max_bid FROM bids WHERE auction_id = {$auction['auction_id']}";
                $stmt = $conn->query($query);
                $maxBid = $stmt->fetch(PDO::FETCH_ASSOC)['max_bid'];
                ?>
                <p class="price">Current bid: £<?= number_format($maxBid, 2); ?></p>
                <a href="auctionPages.php?id=<?php echo $auction['auction_id']; ?>" class="more auctionLink">More &gt;&gt;</a>
            </article>
        </li>
    <?php endforeach; ?>
</ul>
