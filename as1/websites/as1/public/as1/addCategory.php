...
require 'header.php';
// Include the database functions file (assuming it contains functions like addCategory)
require 'db.php';
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        ...
        $success = addCategory($name);

        // Redirect to the adminCategories page if the category is added successfully
        // Otherwise, display an error message
        if ($success) {
            header('Location: adminCategories.php');
            exit();
        } else {
            $addError = 'Failed to add the category. Please try again.';
        }
    } catch (Exception $e) {
         // Log the exception for debugging purposes
        error_log('Error during category addition: ' . $e->getMessage());
