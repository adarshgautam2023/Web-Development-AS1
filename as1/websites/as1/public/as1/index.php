<?php require 'Header.php';
 ?>
		<main>
			...
			<?php
require 'db.php';
$auctions = getLast10Auctions();
function getLast10Auctions() {
    try {
        $conn = db_connect();
        $query = "SELECT * FROM auctions ORDER BY endDate DESC LIMIT 10";
        $stmt = $conn->query($query);
        return $stmt->fetchAll(PDO::FETCH_ASSOC);
    } catch (PDOException $e) {
        error_log('Error during getLast10Auctions: ' . $e->getMessage());
        return false;
    }
}
