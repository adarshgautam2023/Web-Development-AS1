...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        $success = editCategory($categoryId, $newName);

        if ($success) {
            header('Location: adminCategories.php');
            exit();
        } else {
            $editCategoryError = 'Failed to edit category. Please try again.';
        }
    } catch (Exception $e) {
        error_log('Error during category edit: ' . $e->getMessage());
        $editCategoryError = 'An unexpected error occurred. Please try again later.';
    }
}

if (isset($_GET['id'])) {
    $categoryId = $_GET['id'];
    $category2 = getCategoryById($categoryId);
...
<main>
    ...
    <?php if (isset($editCategoryError)) : ?>
        <p class="error-message"><?php echo $editCategoryError; ?></p>
    <?php endif; ?>

    <form action="editCategory.php" method="post">
        <input type="hidden" name="category_id" value="<?php echo $categoryId; ?>">
        <label for="new_name">New Category Name:</label>
        <input type="text" id="new_name" name="new_name" value="<?php echo $category2['name']; ?>" required>
