...
require 'Header.php';
// Include the database functions file (assuming it contains functions like addAuction, getCategories)
require 'db.php';

// Check if the user is logged in by starting the session
session_start();
...
// Process the form submission if the HTTP request method is POST
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($title) || empty($description) || empty($category) || empty($endDate)) {
        ...
        $userId = $_SESSION['user_id'];
         ...
         // Call the addAuction function to add the auction to the database
        $success = addAuction($title, $description, $category, $endDate, $userId);

        // Redirect to the homepage if the auction is added successfully
        // Otherwise, display an error message
        if ($success) {
            header('Location: index.php');
            exit();
        } else {
            $addError = 'Failed to add auction. Please try again.';
        }
    }
}
...
<main>
    ...
    <?php
    // Display an error message if $addError is set
    if (isset($addError)) {
        echo '<p class="error-message">' . $addError . '</p>';
    }
    ?>
...
<!-- Include the common footer file that closes HTML tags and includes additional scripts -->
